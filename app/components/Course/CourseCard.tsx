  ...
  return (
    <Link href={!isProfile ? `/course/${item._id}` : `course-access/${item._id}`}>
        <div className='w-full min-h-[35vh] dark:bg-slate-500 dark:bg-opacity-20 backdrop-blur border dark:border-[#ffffff1d] border-[#00000015] dark:shadow-[bg-slate-700] rounded-lg p-3 shadow-sm dark:shadow-inner'>
            <Image src={item.thumbnail.url} width={500} height={300} objectFit='cover' className='rounded w-full' alt="" />
            <br />
            <h1 className="font-Poppins text-[15px] text-black dark:text-white">
                {item.name}
            </h1>
            <div className={`w-full flex items-center justify-between pt-2`}>
                <Ratings rating={item.ratings}/>
                <h5 className={`text-black dark:text-white ${isProfile && "hidden 800px:inline"}`}>{item.purchased} Students</h5>
            </div>
